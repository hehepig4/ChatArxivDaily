...
class ArxivObj:
    def __init__(self, 
        ...
        self.subjects_list = subjects # https://arxiv.org/category_taxonomy
        self.subjects_list = ["cat:"+subject+"+OR+" for subject in self.subjects_list]
        self.subject_query = "".join(self.subjects_list)[:-4]
        self.max_results = max_results
        self.sort_by = sort_by
        self.client = arxiv.Client()
        self.logger = logger
        
    def search_by_keywords(self,key_words_list,methods="or"):
        paper_list=[]
        for key_word in key_words_list:
            paper_list.append([self.client.search("all:"+key_word+"+AND+"+self.subject_query,
                                                    max_results=self.max_results,
                                                    sort_by=self.sort_by)])
            self.logger.info("searching by keyword: {}".format(key_word))
        if methods=="or":
            paper_list = [item for sublist in paper_list for item in sublist]
            paper_list = list(set(paper_list))
        elif methods=="and":
            paper_sets = [set(paper) for paper in paper_list]
            paper_list = list(set.intersection(*paper_sets))
        self.logger.info("method: {}, total papers: {}".format(methods,len(paper_list)))
        return paper_list
        
    def return_ids(self,paper_list):
        return [paper.id for paper in paper_list]
    
    def return_title_author_abs(self,paper_list):
        return [(paper.title,paper.authors,paper.summary) for paper in paper_list]

    def download_with_ids(self,ids_list,download_path):
        for id in ids_list:
            self.logger.info("downloading paper: {}".format(id))
            self.client.download(id,download_path)   
class ChatGPT:
    pass